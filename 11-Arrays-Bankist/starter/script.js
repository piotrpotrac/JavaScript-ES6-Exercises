...
//2.
console.log(dogs.find(dog => dog.owners.join(' ').includes('Sarah')));
...
const q = ownersEatTooMuch.map(owner => owner.owners.slice().join(' and '));
console.log(`${q.join(' and ')} dogs eat too much!`);

const x = ownersEatTooLittle.map(owner => owner.owners.join(' and '));
console.log();

console.log(`${x.join(' and ')} dogs eat too little!`);
